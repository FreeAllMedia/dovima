...
describe("Model.find.one", function () {
  ...
  it("should return a single record", function (done) {
    var mockRecord = { id: 1, name: "Bob" };
    User.database.mock({
      "select * from `users` where `id` = 1 limit 1": [mockRecord]
    });

    User.find.one.where("id", mockRecord.id).results(function (error, user) {
      if (error) {
        throw error;
      }
