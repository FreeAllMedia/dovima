...
describe("Model.find.andWhere(...options)", function () {
  ...
  it("should add an and clause to the query conditionals", function (done) {
    var query = "select * from `users` where `some_id` = 1 and `last_name` = 'Jones'";
    var querySpy = User.database.spy(query);

    User.find.where("someId", 1).andWhere("last_name", "Jones").results(function () {
