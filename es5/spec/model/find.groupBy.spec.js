...
describe("Model.find.groupBy(columnName)", function () {
  ...
  it("should add an group by clause to the query", function (done) {
    var query = "select * from `users` group by `age`";
    var querySpy = User.database.spy(query);

    User.find.groupBy("age").results(function () {
      querySpy.callCount.should.eql(1);
      done();
    });
  });

  it("should add multiple group by clauses to the query", function (done) {
    var query = "select * from `users` group by `age`, `last_name`";
    var querySpy = User.database.spy(query);

    User.find.groupBy("age", "last_name").results(function () {
      querySpy.callCount.should.eql(1);
      done();
    });
  });
